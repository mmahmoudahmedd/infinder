# Quick Wizard Improvements Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Improve the Quick Wizard with 8 enhancements: Sharia step, amount step, summary screen, back button, risk descriptions, dynamic label, result explanation, and animated transitions.

**Architecture:** Backend gains Sharia-adjusted allocation templates, dynamic label, and explanation text in the `/robo` endpoint. Frontend wizard expands from 3 steps to 5 steps + summary + result, all in `SmartAssistant.tsx`. Framer Motion (already installed) handles slide animations.

**Tech Stack:** React, TypeScript, Framer Motion, Express (Node.js ESM)
...
| File | Change |
|---|---|
| `backend/routes/investments.js` | Add Sharia templates, dynamic label/explanation to `/robo` |
| `frontend/src/pages/SmartAssistant.tsx` | Full wizard rewrite — 5 steps + summary + result |

---

## Task 1: Update backend robo endpoint

**Files:**
- Modify: `backend/routes/investments.js`

- [ ] **Step 1: Replace `ROBO_TEMPLATES` and the `router.post('/robo', ...)` handler**

Open `backend/routes/investments.js`. Replace the `ROBO_TEMPLATES` constant and the `/robo` route with the following. Do NOT touch any other part of the file.

Replace this block (lines 8–48):
```js
...
- [ ] **Step 2: Verify the backend is running without errors**

```bash
# In backend/ directory — restart if already running
npm run dev
```

Expected: `INFINDER API listening on 4000` with no errors.

- [ ] **Step 3: Commit**

```bash
git add backend/routes/investments.js
git commit -m "feat: add Sharia templates and dynamic label/explanation to robo endpoint"
```

---

## Task 2: Rewrite SmartAssistant.tsx wizard
...
Replace `frontend/src/pages/SmartAssistant.tsx` with:
...
Expected: Build succeeds with no TypeScript errors. If you see `motion` not found, run `npm install framer-motion` (it should already be installed — check with `npm list framer-motion`).

- [ ] **Step 3: Start dev server and test the wizard manually**

```bash
npm run dev
```

Open http://localhost:5173/assistant and click "Quick wizard". Verify:
- Step 0 appears with Goal options
- Clicking a goal slides to Step 1
- Back button returns to previous step
- Progress bar fills as you advance
- Step 2 shows risk descriptions under each option
- Step 3 asks about Sharia preference
- Step 4 asks for amount, Continue button is disabled until amount > 0
- Step 5 shows summary of all answers
- Clicking "Get my portfolio" calls the API and shows Step 6 with pie chart
- Dynamic label matches your inputs (e.g. "Long-term Growth Halal Portfolio")
- Explanation text appears below the label
- "Invest now" submits the investment

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/SmartAssistant.tsx
git commit -m "feat: improve wizard with 5 steps, back button, Sharia, summary, animations, dynamic label"
```
