...
**Goal:** Rewrite `frontend/src/pages/LearningHub.tsx` so each course carries its own `curriculum[]`, state variables match the spec (`currentView`, `selectedCourse`, `selectedLesson`), and the progress bar is computed dynamically.

**Architecture:** Single file, single parent component (`LearningHub`) owns all state and passes data down as props to three stateless screen components (`HubScreen`, `DetailScreen`, `CurriculumScreen`). `framer-motion` `AnimatePresence` handles slide transitions; direction is tracked via a `useRef`.

**Tech Stack:** React 18, TypeScript, Tailwind CSS, framer-motion 11, react-router-dom 6
...
| Action | Path |
|--------|------|
| Overwrite | `frontend/src/pages/LearningHub.tsx` |

No other files change.

---

## Task 1: Write the complete rewritten LearningHub.tsx

**Files:**
- Overwrite: `frontend/src/pages/LearningHub.tsx`

This is a full file replacement. Write the file exactly as shown below.

- [ ] **Step 1: Replace the file contents**

Open `frontend/src/pages/LearningHub.tsx` and replace everything with:
    ...
    <div className="px-4 pt-7 pb-6">
      ...
      <div className="mt-6 space-y-4">
        {COURSES.map((course, i) => (
          ...
          >
            {/* Coloured banner — uses course.color */}
...
function DetailScreen({
  ...
  return (
    <div className="pb-4">
      ...
      {/* Hero banner — dark bg with course-colour glow */}
...
export default function LearningHub() {
  ...
  // selectedLesson is part of the state contract; used by future lesson-player integration
  void selectedLesson;
...
Expected: no output (zero errors). If errors appear, check that all types referenced in the file match the definitions at the top of the file (`Course`, `CurriculumItem`, `CurrentView`, `LessonStatus`).

- [ ] **Step 3: Commit**

```bash
git add frontend/src/pages/LearningHub.tsx
git commit -m "feat: rewrite LearningHub with per-course curriculum and dynamic progress"
```
...
Navigate to the `/learn` route in a browser at ~390px width (DevTools mobile view).

- [ ] **Step 2: Hub screen**

Verify:
- Three course cards render (Startups green, Real Estate blue, Investment purple banners)
- "Start Course" button is visible on each card

- [ ] **Step 3: Hub → CourseDetail**

Click "Start Course" on the **Startups** card. Verify:
- Slide-in animation plays (left to right)
- Hero banner is dark with a green glow tint
- "Current Lesson" pill shows "Term Sheets & Valuations"
- Meta row shows "5 Lessons" and "3h 20m Total"
- Progress bar animates to **40%** (2 of 5 completed)
- Course overview text is the Startups overview paragraph

- [ ] **Step 4: CourseDetail → Curriculum**

Tap "View Curriculum" or the play button. Verify:
- Curriculum screen slides in
- Progress card shows **40%** for Startups
- Lesson 1 "Introduction to Venture Capital" — green checkmark circle
- Lesson 2 "Deal Flow & Due Diligence" — green checkmark circle
- Lesson 3 "Term Sheets & Valuations" — green play circle, green left border, light green row bg
- Lessons 4 & 5 — grey play circle, row is dim (opacity-60)

- [ ] **Step 5: Curriculum → CourseDetail → Hub**

Tap back arrow from Curriculum → verifies slide-back to CourseDetail. Tap "Back to Courses" → verifies slide-back to Hub, all 3 course cards still visible.

- [ ] **Step 6: Repeat for a second course**

Click "Start Course" on **Real Estate** card. Verify:
- Hero banner glow is blue-tinted
- Progress bar is **20%** (1 of 5 completed)
- Curriculum: Lesson 1 checkmark, Lesson 2 current (green border), Lessons 3-5 locked

- [ ] **Step 7: Bottom nav**

On any screen, verify Learn tab is green/active. Tap Home → navigates to `/dashboard`. Tap Trade or Invest → navigates to `/invest`.
